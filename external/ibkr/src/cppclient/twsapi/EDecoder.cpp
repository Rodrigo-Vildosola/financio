...
const char* EDecoder::processOpenOrderMsgProtoBuf(const char* ptr, const char* endPtr) {
	...
	Order order;
	if (openOrderProto.has_order()) order = EDecoderUtils::decodeOrder(openOrderProto.contract(), openOrderProto.order());
	OrderState orderState;
...
const char* EDecoder::processExecutionDetailsMsgProtoBuf(const char* ptr, const char* endPtr) {
	...
	int reqId = executionDetailsProto.has_reqid() ? executionDetailsProto.reqid() : 0;
	Contract contract;
...
const char* EDecoder::processExecutionDetailsEndMsgProtoBuf(const char* ptr, const char* endPtr) {
	...
	int reqId = executionDetailsEndProto.has_reqid() ? executionDetailsEndProto.reqid() : 0;
