...
void logger::init()
{
    ...
    log_sinks.emplace_back(std::make_shared<spdlog::sinks::basic_file_sink_mt>("eng.log", true));

    log_sinks[0]->set_pattern("%^[%T] %n: %v%$");
    log_sinks[1]->set_pattern("[%T] [%l] %n: %v");

    core_logger = std::make_shared<spdlog::logger>("TRINIO", begin(log_sinks), end(log_sinks));
    ...
    only_file_logger = std::make_shared<spdlog::logger>("TR_F", log_sinks[1]);
